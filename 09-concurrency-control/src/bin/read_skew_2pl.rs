...
fn audit(store: &Arc<Mutex<HashMap<String, i64>>>, locks: &Arc<HashMap<String, RwLock<()>>>) {
    let _tx_x = locks.get("x").unwrap().read().unwrap();
    let _tx_y = locks.get("y").unwrap().read().unwrap();
